...
//Change the color of active button
document.querySelectorAll('.nav-btn').forEach((btn) => {
  btn.addEventListener("click", (event) => {
    event.currentTarget.classList.toggle("current-link")
  });
});
...
function render_email_row(email) {
  ...
  item.setAttribute('class', 'mail-item');
  ...
  if (!email.read) {
    row.style.backgroundColor = '#F5F7F7'
  } else {
  ...
  var d = new Date(Date.parse(email.timestamp));
  ...
  if (year === today.getFullYear() && month === today.getMonth() + 1 && day === today.getDate()) {
    timestamp.innerHTML = hour + " : " + min
  // Else, show only the day and month :
  } else {
    timestamp.innerHTML = year + " / " + month + " / " + day
  }
...
function add_archive_button(email_id) {
  ...
  if (mailbox !== 'Sent') {
    ...
    archive_button.setAttribute('class', `btn btn-sm btn-flat`);
